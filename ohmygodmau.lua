                ...
                task.spawn(function()
                    while v:IsDescendantOf(game) do
                        ...
                        local succ,err=pcall(function()
                            ...
                            Head.Anchored = true
                            Head.CFrame = Torso.CFrame * CFrame.new(0,1.5*0.5,0)
                            LeftLeg.Anchored = true
                            RightArm.Anchored = true
                            LeftArm.Anchored = true
                            RightArm.CFrame = Torso.CFrame * CFrame.new(1.5*0.5,-0.2*0.5,0)*CFrame.Angles(0,math.rad(24+16*math.sin(SINE/25)),math.rad(8+4*math.sin(SINE/25)))
                            LeftArm.CFrame = Torso.CFrame * CFrame.new(-1.5*0.5,-0.2*0.5,0)*CFrame.Angles(0,math.rad(-24-16*math.sin(SINE/25)),math.rad(-8-4*math.sin(SINE/25)))
                            RightLeg.CFrame = Torso.CFrame * CFrame.new(0.5*0.5,-2*0.5,-0.3)*CFrame.Angles(math.rad(-24+8*math.sin(SINE/25)),0,0)
                            LeftLeg.CFrame = Torso.CFrame * CFrame.new(-0.5*0.5,-2*0.5,-0.3)*CFrame.Angles(math.rad(-24+8*math.sin(SINE/25)),0,0)
                        end) 
                        v.Parent:FindFirstChild("HumanoidRootPart").RootJoint.Enabled = false
                        v.Parent.Torso.Anchored = true
                        v.Parent.Torso.CFrame = CFrame.new(-1.35 * COS(sn / 30), -2 + 0.7 * SIN(sn / 90), 0 + 2 * SIN(sn / 40)) * CFrame.Angles(RAD(0 + 5 * SIN(sn / 60)), RAD(-0 + 7 * SIN(sn / 60)), RAD(-0 + 7 * SIN(sn / 60)))
                    end
                end)
            end 
        end
